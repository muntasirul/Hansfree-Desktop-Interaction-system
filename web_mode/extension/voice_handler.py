...
class WebVoiceHandler:
    ...
    def _parse(self, raw: str):
        ...
        # Typing mode — pass everything as text
        if self._typing_mode:
            self.on_type(self._apply_punct(text))
            return
        ...
        m = re.search(
            r'\b(?:click|number|select|press|open|choose)\s+(\d+|[a-z\s]+)\b',
            lower
        )
        if m:
            n = self._to_int(m.group(1).strip())
            if n is not None:
                self.on_click(n)
                return

        # Bare number
        n = self._to_int(lower.strip())
        if n is not None:
            self.on_click(n)
            return
        ...
        # Search fallback — only if NOT a stop-like phrase
        words = text.split()
        if len(words) >= 2:
            self.on_navigate(
